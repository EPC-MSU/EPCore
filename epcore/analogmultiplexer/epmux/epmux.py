...
def _get_dll() -> CDLL:
    ...
    if os_kind == "windows":
        if 8 * struct.calcsize("P") == 32:
            lib = CDLL(_fullpath_lib(os.path.join("epmux-win32", "epmux.dll")))
        else:
            lib = CDLL(_fullpath_lib(os.path.join("epmux-win64", "epmux.dll")))
    elif os_kind == "freebsd" or "linux" in os_kind:
        if platform.uname()[4] == "aarch64":
            lib = CDLL(_fullpath_lib(os.path.join("epmux-arm64", "libepmux.so")))
        else:
            lib = CDLL(_fullpath_lib(os.path.join("epmux-debian", "libepmux.so")))
    else:
