"""
Project generated by builder 0.10.12 protocol 1.0.0
"""
...
from ctypes import c_ushort, c_ubyte, c_uint
from ctypes import c_wchar_p, c_int, c_void_p, c_char_p, c_size_t
try:
...
urpc_builder_version_bugfix = 12
urpc_builder_version_suffix = ""
urpc_builder_version = "0.10.12"
_Ok = 0
_Error = -1
_NotImplemented = -2
_ValueError = -3
...
def _load_lib():
    ...
    if os_kind == "windows":
        if 8 * struct.calcsize("P") == 32:
            paths = (_near_script_path(os.path.join("epmux-win32", "epmux.dll")), _near_script_path("epmux.dll"),
                     "epmux.dll")
        else:
            paths = (_near_script_path(os.path.join("epmux-win64", "epmux.dll")), _near_script_path("epmux.dll"),
                     "epmux.dll")
    elif os_kind == "freebsd" or "linux" in os_kind:
        paths = (_near_script_path(os.path.join("epmux-debian", "libepmux.so")), _near_script_path("libepmux.so"),
                 "libepmux.so")
    else:
...
class EpmuxDeviceHandle:
    ...
    def reboot_to_bootloader(self) -> None:
        _validate_call(_lib.epmux_reboot_to_bootloader(self._handle))
    ...
    def debug_read(self, **kwargs) -> "DebugReadResponse":
        ...
        _validate_call(_lib.epmux_debug_read(self._handle, byref(dst_buffer)))
        return dst_buffer
    ...
    def debug_write(self, *args) -> None:  # noqa: F811
        ...
        if len(args) != 1 or not isinstance(args[0], self.DebugWriteRequest):
            ...
            src_buffer = args[0]
        _validate_call(_lib.epmux_debug_write(self._handle, byref(src_buffer)))
